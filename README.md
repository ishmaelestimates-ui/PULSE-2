...
Single global settings row — **there's no auth/user system in this app**,
so "per user" settings aren't possible yet; this is one shared
project-wide configuration (primary/secondary/tertiary color, font,
logo, intro/outro music). Add a `user_id` column and drop the singleton
constraint once auth exists.
...
2. **The existing Sprint 1-7 API surface is NOT retrofitted with access
   control in this pass.** Every endpoint from the first seven sprints
   remains open, exactly as before. Auth is wired for the new
   user-management and Fame endpoints only. Retrofitting role-based
   access across ~100 existing endpoints — deciding what an "editor" vs
   "admin" can do on every single one — is a distinct, substantial
   follow-up task, not something to bolt on silently in the same pass
   that introduces the concept of a user at all.
